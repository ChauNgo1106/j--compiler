...
class JPositiveOp extends JUnaryExpression {
    ...
    /**
     * Generating code for the positive operation involves generating code for
     * the operand, and then the negation instruction.
     * 
     * @param output
     *            the code emitter (basically an abstraction for producing the
    ...
    public void codegen(CLEmitter output) {
        arg.codegen(output);
        //negate of negate should be back what the input was. 
        if (arg.type() == Type.INT){
            output.addNoArgInstruction(INEG);
            output.addNoArgInstruction(INEG);
        } else if(arg.type() == Type.DOUBLE){
...
class JComplementOp extends JUnaryExpression {
    ...
    /**
     * Constructs an AST node for a complement expression given its line number,
     * and the operand.
     * 
     * @param line
    ...
    /**
     * Analyzing the complement operation involves analyzing its operand, checking
     * its type and determining the result type.
     * 
     * @param context
     *            context in which names are resolved.
